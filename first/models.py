...
class User(AbstractBaseUser):
    ...
    def start_challenge(self):
        ...
        The guard is `_round_started_at`, not `race_started_at`, so a round
        recorded by the previous CSS-editor version counts as the attempt it
        was: those records carry only `game_start_time`, and reading the newer
        field alone would have handed them a second official attempt.

        Returns True if this call actually started the race.
        """
        if self._round_started_at or self.race_completed_at:
            return False
        now = timezone.now()
    ...
    def is_expired(self):
        ...
        Derived from `race_status` so there is exactly one definition of the
        state machine and the two can never disagree about a record.
        """
        return self.race_status == RACE_EXPIRED
    ...
    def race_status(self):
        """The single definition of where this attempt stands.

        Everything else — `is_expired`, `is_locked`, the race endpoints, the
        scoreboard, the exports — reads the state machine through here, so an
        attempt is never simultaneously "not started" in one place and "timed
        out" in another.

        It asks `_round_started_at`, which counts a round recorded by the
        previous CSS-editor version as a started attempt. Those records hold
        only `game_start_time`; treating them as unstarted used to report them
        as NOT_STARTED while `is_expired` said otherwise, and let them start a
        second official race.
        """
        if self.race_completed_at:
            return RACE_COMPLETED
        if not self._round_started_at:
